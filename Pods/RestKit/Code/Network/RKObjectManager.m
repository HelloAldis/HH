...
static NSArray *RKFilteredArrayOfResponseDescriptorsMatchingPathAndMethod(NSArray *responseDescriptors, NSString *path,
                                                                          RKRequestMethod method) {
  NSIndexSet *indexSet = [responseDescriptors
      indexesOfObjectsPassingTest:^BOOL(RKResponseDescriptor *responseDescriptor, NSUInteger idx, BOOL *stop) {
 ...
 */
RKRequestDescriptor *RKRequestDescriptorFromArrayMatchingObjectAndRequestMethod(NSArray *requestDescriptors, id object,
...
RKRequestDescriptor *RKRequestDescriptorFromArrayMatchingObjectAndRequestMethod(NSArray *requestDescriptors, id object,
  ...
  do {
    for (RKRequestDescriptor *requestDescriptor in requestDescriptors) {
      if ([requestDescriptor.objectClass isEqual:searchClass] && (requestMethod == requestDescriptor.method))
  ...
  if (value) {
    if ([value isKindOfClass:[NSMutableArray class]]) {
      ...
      [NSException raise:NSInvalidArgumentException
                  format:@"Unexpected argument of type '%@': expected an NSDictionary or NSArray.", [value class]];
    }
  } else {
...
- (id)requestParameters {
  ...
  if (valueAtNullKey) {
    if ([self.parameters count] == 1)
      return valueAtNullKey;

    // If we have values at `[NSNull null]` and other key paths, we have an invalid configuration
    [NSException raise:NSInvalidArgumentException
                format:@"Invalid request descriptor configuration: The request descriptors specify that multiple "
                       @"objects be serialized at incompatible key paths. Cannot serialize objects at the `nil` root "
                       @"key path in the same request as objects with a non-nil root key path. Please check your "
...
- (void)visitMapping:(RKMapping *)mapping {
  if ([self.mappings containsObject:mapping])
    return;
...
/**
 Returns `YES` if the given array of `RKResponseDescriptor` objects contains an `RKEntityMapping` anywhere in its object
 graph.

 @param responseDescriptors An array of `RKResponseDescriptor` objects.
 @return `YES` if the `mapping` property of any of the response descriptor objects in the given array is an instance of
 `RKEntityMapping`, else `NO`.
 */
#ifdef RKCoreDataIncluded
...
static BOOL RKDoesArrayOfResponseDescriptorsContainMappingForClass(NSArray *responseDescriptors,
  ...
  for (RKMapping *mapping in accessibleMappings) {
    if ([mapping isKindOfClass:[RKObjectMapping class]]) {
      if ([[(RKObjectMapping *)mapping objectClass] isSubclassOfClass:classToBeMapped])
        return YES;
    }
...
static NSString *RKMIMETypeFromAFHTTPClientParameterEncoding(AFHTTPClientParameterEncoding encoding) {
  ...
  default:
    RKLogWarning(@"RestKit is unable to infer the appropriate request serialization MIME Type from an "
                 @"`AFHTTPClientParameterEncoding` value of %d: defaulting to `RKMIMETypeFormURLEncoded`",
                 encoding);
    break;
  }
...
- (NSMutableURLRequest *)requestWithMethod:(NSString *)method
  ...
  if (parameters &&
      !([method isEqualToString:@"GET"] || [method isEqualToString:@"HEAD"] || [method isEqualToString:@"DELETE"])) {
    ...
    NSString *charset = (__bridge NSString *)CFStringConvertEncodingToIANACharSetName(
        CFStringConvertNSStringEncodingToEncoding(self.HTTPClient.stringEncoding));
    [request setValue:[NSString stringWithFormat:@"%@; charset=%@", self.requestSerializationMIMEType, charset]
        forHTTPHeaderField:@"Content-Type"];
    NSData *requestBody =
        [RKMIMETypeSerialization dataFromObject:parameters MIMEType:self.requestSerializationMIMEType error:&error];
...
- (NSMutableURLRequest *)requestWithPathForRelationship:(NSString *)relationship
  ...
  NSAssert(URL, @"No relationship route found for the '%@' class with the name '%@'", NSStringFromClass([object class]),
           relationship);
  return [self requestWithMethod:RKStringFromRequestMethod(method) path:[URL relativeString] parameters:parameters];
}
...
- (id)mergedParametersWithObject:(id)object method:(RKRequestMethod)method parameters:(NSDictionary *)parameters {
  ...
  for (id objectToParameterize in objectsToParameterize) {
    RKRequestDescriptor *requestDescriptor = RKRequestDescriptorFromArrayMatchingObjectAndRequestMethod(
        self.requestDescriptors, objectToParameterize, method);
    if ((method != RKRequestMethodGET && method != RKRequestMethodDELETE) && requestDescriptor) {
      NSError *error = nil;
      NSDictionary *parametersForObject = [RKObjectParameterization parametersWithObject:objectToParameterize
                                                                       requestDescriptor:requestDescriptor
                                                                                   error:&error];
      if (error) {
        RKLogError(@"Object parameterization failed while building %@ request for object '%@': %@",
                   RKStringFromRequestMethod(method), objectToParameterize, error);
        return nil;
      }
  ...
  if ([requestParameters isKindOfClass:[NSArray class]] && parameters) {
    [NSException
        ...
        format:@"Cannot merge parameters with array of object representations serialized with a nil root key path."];
...
- (NSMutableURLRequest *)multipartFormRequestWithObject:(id)object
  ...
  NSMutableURLRequest *multipartRequest =
      [self.HTTPClient multipartFormRequestWithMethod:RKStringFromRequestMethod(method)
                                                 path:requestPath
...
- (Class)requestOperationClassForRequest:(NSURLRequest *)request fromRegisteredClasses:(NSArray *)registeredClasses {
  ...
  while (requestOperationClass = [enumerator nextObject]) {
    if ([requestOperationClass canProcessRequest:request])
      break;
...
- (RKObjectRequestOperation *)objectRequestOperationWithRequest:(NSURLRequest *)request
                                                        ...
                                                        failure:(void (^)(RKObjectRequestOperation *operation,
                                                                          NSError *error))failure {
  return [self objectRequestOperationWithRequest:request
...
- (RKObjectRequestOperation *)objectRequestOperationWithRequest:(NSURLRequest *)request
                                            responseDescriptors:(NSArray *)responseDescriptors
                                                        ...
                                                        failure:(void (^)(RKObjectRequestOperation *operation,
                                                                          NSError *error))failure {
  Class HTTPRequestOperationClass =
      [self requestOperationClassForRequest:request fromRegisteredClasses:self.registeredHTTPRequestOperationClasses]
          ?: [RKHTTPRequestOperation class];
  ...
  Class objectRequestOperationClass =
      [self requestOperationClassForRequest:request fromRegisteredClasses:self.registeredObjectRequestOperationClasses]
          ?: [RKObjectRequestOperation class];
  RKObjectRequestOperation *operation =
      [[objectRequestOperationClass alloc] initWithHTTPRequestOperation:HTTPRequestOperation
...
- (RKManagedObjectRequestOperation *)
    managedObjectRequestOperationWithRequest:(NSURLRequest *)request
...
- (RKManagedObjectRequestOperation *)
  ...
  [self copyStateFromHTTPClientToHTTPRequestOperation:HTTPRequestOperation];
  Class objectRequestOperationClass =
      [self requestOperationClassForRequest:request
                      fromRegisteredClasses:self.registeredManagedObjectRequestOperationClasses]
          ?: [RKManagedObjectRequestOperation class];
...
- (id)appropriateObjectRequestOperationWithObject:(id)object
  ...
  NSArray *matchingDescriptors =
      RKFilteredArrayOfResponseDescriptorsMatchingPathAndMethod(self.responseDescriptors, path, method);
  ...
  if (isManagedObjectRequestOperation && !self.managedObjectStore)
    RKLogWarning(@"Asked to create an `RKManagedObjectRequestOperation` object, but managedObjectStore is nil.");
  if (isManagedObjectRequestOperation && self.managedObjectStore) {
    // Construct a Core Data operation
    NSManagedObjectContext *managedObjectContext = [object respondsToSelector:@selector(managedObjectContext)]
                                                       ? [object managedObjectContext]
                                                       : self.managedObjectStore.mainQueueManagedObjectContext;
    ...
    if ([object isKindOfClass:[NSManagedObject class]]) {
      ...
      if (!temporaryObjectsPredicate)
        temporaryObjectsPredicate = [NSPredicate predicateWithFormat:@"objectID.isTemporaryID == YES"];
      NSSet *temporaryObjects =
          [[managedObjectContext insertedObjects] filteredSetUsingPredicate:temporaryObjectsPredicate];
      if ([temporaryObjects count]) {
        RKLogInfo(@"Asked to perform object request for NSManagedObject with temporary object IDs: Obtaining permanent "
                  @"ID before proceeding.");
        __block BOOL _blockSuccess;
        __block NSError *_blockError;

        [[object managedObjectContext] performBlockAndWait:^{
            _blockSuccess = [[object managedObjectContext] obtainPermanentIDsForObjects:[temporaryObjects allObjects]
                                                                                  error:&_blockError];
        }];
        if (!_blockSuccess)
          RKLogWarning(@"Failed to obtain permanent ID for object %@: %@", object, _blockError);
      }
    }
  } else {
    // Non-Core Data operation
    operation = [self objectRequestOperationWithRequest:request
                                    responseDescriptors:matchingDescriptors
                                                success:nil
                                                failure:nil];
  }
  ...
  //    if (RKDoesArrayOfResponseDescriptorsContainMappingForClass(self.responseDescriptors, [object class]))
  //    operation.targetObject = object;
  operation.mappingMetadata = routingMetadata;
  return operation;
}

- (NSURL *)URLWithRoute:(RKRoute *)route
                    object:(id)object
    interpolatedParameters:(NSDictionary **)interpolatedParameters {
  NSString *path = nil;
  if (object) {
    RKPathMatcher *pathMatcher = [RKPathMatcher pathMatcherWithPattern:route.pathPattern];
    path = [pathMatcher pathFromObject:object
                         addingEscapes:route.shouldEscapePath
                interpolatedParameters:interpolatedParameters];
  } else {
...
- (void)getObjectsAtPathForRelationship:(NSString *)relationshipName
                             ...
                             parameters:(NSDictionary *)parameters
                                ...
                                failure:(void (^)(RKObjectRequestOperation *operation, NSError *error))failure {
  RKRoute *route =
      [self.router.routeSet routeForRelationship:relationshipName ofClass:[object class] method:RKRequestMethodGET];
  NSDictionary *interpolatedParameters = nil;
  NSURL *URL = [self URLWithRoute:route object:object interpolatedParameters:&interpolatedParameters];
  NSAssert(URL, @"Failed to generate URL for relationship named '%@' for object: %@", relationshipName, object);
  RKObjectRequestOperation *operation = [self appropriateObjectRequestOperationWithObject:nil
...
- (void)getObjectsAtPathForRouteNamed:(NSString *)routeName
  ...
  NSURL *URL = [self URLWithRoute:route object:object interpolatedParameters:&interpolatedParameters];
  NSAssert(URL, @"No route found named '%@'", routeName);
  NSAssert(route.method & RKRequestMethodGET, @"Expected route named '%@' to specify a GET, but it does not",
           routeName);
...
- (void)getObjectsAtPath:(NSString *)path
  ...
  RKObjectRequestOperation *operation =
      [self appropriateObjectRequestOperationWithObject:nil method:RKRequestMethodGET path:path parameters:parameters];
  [operation setCompletionBlockWithSuccess:success failure:failure];
  [self enqueueObjectRequestOperation:operation];
}
...
- (RKPaginator *)paginatorWithPathPattern:(NSString *)pathPattern {
  ...
  RKPaginator *paginator = [[RKPaginator alloc] initWithRequest:request
                                              paginationMapping:self.paginationMapping
                                            responseDescriptors:self.responseDescriptors];
#ifdef RKCoreDataIncluded
  ...
  if (HTTPOperationClass)
    [paginator setHTTPOperationClass:HTTPOperationClass];
  return paginator;
}
...
- (void)addRequestDescriptor:(RKRequestDescriptor *)requestDescriptor {
  ...
  if ([self.requestDescriptors containsObject:requestDescriptor])
    return;
  NSAssert([requestDescriptor isKindOfClass:[RKRequestDescriptor class]],
           @"Expected an object of type RKRequestDescriptor, got '%@'", [requestDescriptor class]);
  [self.requestDescriptors
      enumerateObjectsUsingBlock:^(RKRequestDescriptor *registeredDescriptor, NSUInteger idx, BOOL *stop) {
          NSAssert(!([registeredDescriptor.objectClass isEqual:requestDescriptor.objectClass] &&
                     (requestDescriptor.method == registeredDescriptor.method)),
                   @"Cannot add request descriptor: An existing descriptor is already registered for the class '%@' "
                   @"and HTTP method'%@'.",
                   requestDescriptor.objectClass, RKStringDescribingRequestMethod(requestDescriptor.method));
      }];
...
- (void)addResponseDescriptor:(RKResponseDescriptor *)responseDescriptor {
  ...
  NSAssert([responseDescriptor isKindOfClass:[RKResponseDescriptor class]],
           @"Expected an object of type RKResponseDescriptor, got '%@'", [responseDescriptor class]);
...
- (NSArray *)enqueuedObjectRequestOperationsWithMethod:(RKRequestMethod)method
                                   matchingPathPattern:(NSString *)pathPattern {
  ...
  for (NSOperation *operation in [self.operationQueue operations]) {
    ...
    if ((method & operationMethod) &&
        [pathMatcher matchesPath:pathAndQueryString tokenizeQueryStrings:NO parsedArguments:nil]) {
...
- (void)cancelAllObjectRequestOperationsWithMethod:(RKRequestMethod)method matchingPathPattern:(NSString *)pathPattern {
  for (RKObjectRequestOperation *operation in
       [self enqueuedObjectRequestOperationsWithMethod:method matchingPathPattern:pathPattern]) {
    [operation cancel];
  }
}
...
- (void)enqueueBatchOfObjectRequestOperationsWithRoute:(RKRoute *)route
  ...
  for (id object in objects) {
    ...
    if ([route isClassRoute]) {
      operation = [self appropriateObjectRequestOperationWithObject:object
                                                             method:route.method
                                                               path:[URL relativeString]
                                                         parameters:nil];
    } else {
      operation = [self appropriateObjectRequestOperationWithObject:nil
                                                             method:route.method
                                                               path:[URL relativeString]
                                                         parameters:nil];
    }
